...
export function shapesDocument (): Record<string, unknown> {
  return {
    ...
    analytics_results: {
      ...
      semantic_contract: 'The bundled catalog is the authoritative table, join, field, and aggregation allowlist for this CLI. Every column discovery result declares its aggregation kind, unit, allowed aggregates, guidance, and required dimensions. UNKNOWN_DATA_REFERENCE and INCOMPATIBLE_GRAIN fail locally with exit 2 before execution, including for nested formulas and CSV; there is no override. Run `poki data describe aggregation` for the complete rule.',
      game_event_terminology: 'Communicate Game Events using frontend Category, What, and Action. Queries and API payloads use backend fields category, action, and label respectively; run `poki data describe game-events` for the machine-readable mapping and lifecycle normalization rule.',
...
export function workflowsDocument (): Record<string, unknown> {
  return {
    ...
    discovery: {
      ...
      event_keys: "Query dbt_p4d_game_events_funnel_v2.event and pass each value verbatim to --event. Communicate keys in frontend terms as Category^What^Action; the analytics model builds that order from backend fields category^action^label. The empty frontend Action form still ends in '^'.",
    ...
    reporting: {
      steps: [
        ...
        'poki data recipe NAME shows required typed parameters; poki data run NAME --team TEAM_ID --last-days 7 --validate-only resolves parameters and checks local structure plus bundled aggregation semantics without claiming API validity.',
      ...
      dates: `Analytics dates are ${ANALYTICS_TIME_ZONE} calendar dates with DST; --last-days N computes the N complete days ending yesterday for date and datetime recipes. Datetime ranges use 00:00:00 through 23:59:59 in that timezone.`
    },
