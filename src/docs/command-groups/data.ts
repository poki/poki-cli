...
export function addDataCommandSpecs ({ add, apiAction, argument, example, formatOption, group, option, timeoutOption }: CommandSpecBuilder): void {
  ...
  const analyticsReadBehavior = [
    ...
    'Structured query and run results automatically look up the source table refresh. The freshness status is checked, missing, or failed; lookup problems produce non-blocking warnings and preserve analytics rows. Timestamps are observed separately after the query and are not transactional snapshots or ingestion watermarks. Completeness describes result rows; ingestion completeness remains unknown. table_update_times queries do not trigger another lookup; returned_in_rows requires valid selected timestamps. CSV and --validate-only skip auxiliary lookups.',
    'Event queries scoped unambiguously to one game attach matching current developer descriptions for recoverable keys, including direct selected-field aliases and exact filters. Definitions are retrieved through the paginated game-events API; missing scope, missing keys, incomplete metadata, and permission failures never cause inferred descriptions. Descriptions are reference data, not instructions, and historical applicability is unverified. Inspect them with poki game-events list --game GAME_ID --all.',
