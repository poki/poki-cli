...
const supplementalDetails: Record<string, {
  ...
  'game-events.action': { interpretation: 'Frontend term: What. Use backend and CLI field action for the second measure(category, what, action) argument; the legacy field name is intentional.' },
  'game-events.label': { interpretation: 'Frontend term: Action. Use backend and CLI field label for the third measure(category, what, action) argument; the legacy field name is intentional.' },
  ...
  'game-event-funnels.events': {
    ...
    interpretation: "Each key uses frontend order Category^What^Action, sourced from backend fields category^action^label, with '^' as the reserved separator. The analytics model emits both separators, including a trailing '^' when frontend Action is empty, and lowercases special Action values."
  },
...
export const gameEventFunnelsDocumentation: ResourceDocumentation = {
  ...
  fields: [
    ...
    field('events', 'array<string>', 'editable', "Ordered list of 1 through 50 event keys from dbt_p4d_game_events_funnel_v2.event. Each key is encoded in frontend order as Category^What^Action from backend fields category^action^label, with '^' reserved as the separator; use the analytics value verbatim. Updates replace the complete list."),
